 ...
 * 
 * <p>
 * Server applications are wrappers around the server classes (naming and storage) that allow the servers to be run from the
 * console. This class provides functionality that is common to both server applications: namely, graceful termination.
 * 
 * <p>
 * In each application, the server runs until the end of file is read from standard input, or until the server stops on its own
 * due to an exception. Server applications also forcefully terminate the JVM if the end of file has been read on standard input,
 * but the server object takes too long to stop.
 * 
 * <p>
 * Derived application classes override the <code>serverType</code>, <code>startServer</code>, and <code>stopServer</code> methods
 * to deal with the particular kind of server used in the application. The <code>stop</code> method of each server should be
 * overridden to call <code>startTerminationTimer</code>, before calling the superclass implementation. The <code>stopped</code>
 * method should be overridden to call <code>serverStopped</code>.
 * 
 * <p>
 * Derived classes should create a new object of their own type immediately upon entry into <code>main</code>, and call that
 * object's <code>run</code> method.
 */
abstract class ServerApplication extends Application {
    ...
    /**
     * If the server stopped due to an exception, gives the exception. Otherwise, set to <code>null</code>.
     */
    ...
    /**
     * Returns a string indicating the kind of server being run in the application - either <code>"naming"</code> or
     * <code>"storage"</code>.
     */
     ...
     *             If the command line arguments passed to the application are malformed. The message string of the exception
     *             object is printed to the console before the application is terminated.
     * @throws Throwable
     *             This method is allowed to throw any exception. The exception will be reported on the console, and the
     ...
     * 
     * <p>
     * This is the non-static replacement for the <code>main</code> method. It starts the server, starts a thread to wait for
     * end-of-file for standard input, and waits until the server has terminated. Once the server terminates, this method prints a
     * message indicating what kind of termination has occurred.
     * 
     * @param arguments
     *            The command line arguments passed to <code>main</code>.
     */
    @Override
    protected void run(String[] arguments) {
        ...
        if (stop_cause == null) {
            ...
            System.err.println(serverType() + " server stopped due to an " + "exception");
    ...
    protected void startTerminationTimer() {
        termination_timeout_timer.schedule(new TerminationTimeoutTask(), TERMINATION_TIMEOUT);
    }
     ...
     * 
     * @param cause
     *            Exception that caused the server to stop, or <code>null</code> if the server stopped normally.
     */
    ...
    private class TerminationTimeoutTask extends TimerTask {
        ...
        public void run() {
            ...
            synchronized (ServerApplication.this) {
                ...
                System.err.println(serverType() + " server timed out while " + "stopping");
                System.exit(EXIT_FAILURE);
            }
        }
    }
    ...
    protected class BadUsageException extends Exception {
        /**
         * Creates a <code>BadUsageException</code> with the given usage string.
         */
