 ...
 * 
 * <p>
 * The string representation of paths is a forward-slash-delimited sequence of path components. The root directory is represented
 * as a single forward slash.
 * 
 * <p>
 * The colon (<code>:</code>) and forward slash (<code>/</code>) characters are not permitted within path components. The forward
 * slash is the delimeter, and the colon is reserved as a delimeter for application use.
 */
public class Path implements Iterable<String>, Comparable<Path>, Serializable {
     ...
     *             If <code>component</code> includes the separator, a colon, or <code>component</code> is the empty string.
     */
    public Path(Path path, String component) {
        // check for illegal argument conditions
        if ((component.indexOf('/') != -1) || (component.indexOf(':') != -1) || (component.length() == 0)) {
            throw new IllegalArgumentException("Component includes the separator" + ", a colon or is the empty string");
        }
        pathComponents = new LinkedList<String>();
        // add existing path
     ...
     * 
     * <p>
     * The string is a sequence of components delimited with forward slashes. Empty components are dropped. The string must begin
    ...
    /**
     * Creates a new path from a pathComponents LinkedList. Helpful for later methods.
     * 
     * @param pathComponents
     *            The linked list of path components.
     */
     ...
     * 
     * <p>
     * The iterator cannot be used to modify the path object - the <code>remove</code> method is not supported.
     * 
     * @return The iterator.
     */
    @Override
     ...
     * @throws IllegalArgumentException
     *             If <code>directory</code> exists but does not refer to a directory.
     */
    ...
    private static void listHelper(Path prefix, File directory, ArrayList<Path> filepaths) throws FileNotFoundException {
        if (!directory.exists()) {
     ...
     * 
     * @return <code>true</code> if the path does represent the root directory, and <code>false</code> if it does not.
     */
     ...
     * 
     * @throws IllegalArgumentException
     *             If the path represents the root directory, and therefore has no parent.
     */
    public Path parent() {
        // check if root
        if (this.isRoot()) {
            throw new IllegalArgumentException("Path represents the root" + "directory and has no parent");
        }
     ...
     * 
     * @throws IllegalArgumentException
     *             If the path represents the root directory, and therefore has no last component.
     */
    public String last() {
        // check if root
        if (this.isRoot()) {
     ...
     * 
     * <p>
     * The other path is a subpath of this path if it is a prefix of this path. Note that by this definition, each path is a
     * subpath of itself.
     * 
     * @param other
     *            The path to be tested.
     * @return <code>true</code> If and only if the other path is a subpath of this path.
     */
     ...
     * 
     * @param root
     *            The resulting <code>File</code> object is created relative to this directory.
     ...
     * 
     * <p>
     * An ordering upon <code>Path</code> objects is provided to prevent deadlocks between applications that need to lock multiple
     * filesystem objects simultaneously. By convention, paths that need to be locked simultaneously are locked in increasing
     * order.
     * 
     * <p>
     * Because locking a path requires locking every component along the path, the order is not arbitrary. For example, suppose
     * the paths were ordered first by length, so that <code>/etc</code> precedes <code>/bin/cat</code>, which precedes
     * <code>/etc/dfs/conf.txt</code>.
     * 
     * <p>
     * Now, suppose two users are running two applications, such as two instances of <code>cp</code>. One needs to work with
     * <code>/etc</code> and <code>/bin/cat</code>, and the other with <code>/bin/cat</code> and <code>/etc/dfs/conf.txt</code>.
     * 
     * <p>
     * Then, if both applications follow the convention and lock paths in increasing order, the following situation can occur: the
     * first application locks <code>/etc</code>. The second application locks <code>/bin/cat</code>. The first application tries
     * to lock <code>/bin/cat</code> also, but gets blocked because the second application holds the lock. Now, the second
     * application tries to lock <code>/etc/dfs/conf.txt</code>, and also gets blocked, because it would need to acquire the lock
     * for <code>/etc</code> to do so. The two applications are now deadlocked.
     * 
     * @param other
     *            The other path.
     * @return Zero if the two paths are equal, a negative number if this path precedes the other path, or a positive number if
     *         this path follows the other path.
     */
    @Override
     ...
     * 
     * <p>
     * The string may later be used as an argument to the <code>Path(String)</code> constructor.
     * 
