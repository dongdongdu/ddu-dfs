 ...
 * 
 * <p>
 * Write calls on a <code>DFSOutputStream</code> are directed to a storage server hosting the given file. Each call corresponds to
 * one network request. If this is not desirable, the <code>DFSOutputStream</code> should be wrapped in a
 * <code>BufferedOutputStream</code> object.
 * 
 * <p>
 * Creating a <code>DFSOutputStream</code> for a file does not cause the file to be created or truncated. The file must exist, and
 * the existing file data is left in place. Writes to the stream cause file data to be overwritten, starting from the beginning of
 * the file.
 */
public class DFSOutputStream extends OutputStream {
    ...
    /**
     * Creates a <code>DFSOutputStream</code> for a file listed by the given naming server.
     * 
     * <p>
     ...
     *             metadata.
     */
    public DFSOutputStream(Service naming_server, Path file) throws FileNotFoundException, IOException {
        // Retrieve a stub for the storage server hosting the file.
        try {
            log("path file is " + file);
     ...
     * 
     * <p>
     * The file should be locked on the naming server for exclusive access. The naming server is contacted on the default client
     * interface port.
     * 
     * @param hostname
     ...
     *             metadata.
     */
    public DFSOutputStream(String hostname, Path file) throws FileNotFoundException, IOException {
     ...
     *             metadata.
     */
    public DFSOutputStream(Service naming_server, String filename) throws FileNotFoundException, IOException {
     ...
     *             metadata.
     */
    public DFSOutputStream(String hostname, String filename) throws FileNotFoundException, IOException {
     ...
     * 
     * <p>
     * The stream is marked as closed. Further attempts to use the output stream will result in <code>IOException</code>.
     */
    @Override
     ...
     * @param buffer_offset
     *            Offset into the buffer from which bytes are to be written.
     * @param write_length
     *            Number of bytes to be written.
     * @throws IOException
     *             If the stream is closed, if the storage server cannot be contacted, or if a write error occurs on the storage
     *             server.
     * @throws NullPointerException
     *             If <code>buffer</code> is <code>null</code>.
     * @throws IndexOutOfBoundsException
     *             If <code>buffer_offset</code> or <code>write_length</code> is negative, or if <code>buffer_offset +
                                          write_length</code> exceeds the
     *             length of the given buffer.
     */
    @Override
    public void write(byte[] buffer, int buffer_offset, int write_length) throws IOException {
     ...
     * 
     * @param b
     *            Value of the byte to be written. The argument is taken modulo <code>256</code> to obtain the byte value.
     * @throws IOException
     *             If the stream is closed, if the storage server cannot be contacted, or if a write error occurs on the storage
     *             server.
     */
     ...
     * 
     * @param count
     *            Number of bytes to advance stream offset by.
     * @throws IOException
