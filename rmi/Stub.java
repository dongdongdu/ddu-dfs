...
public abstract class Stub {
    ...
    public static <T> T create(Class<T> c, Skeleton<T> skeleton) throws UnknownHostException {
        ...
        if ((skeleton.sockaddr.getAddress().isAnyLocalAddress()) && ((Integer) skeleton.sockaddr.getPort() != null)
                && (skeleton.sockaddr.getHostName() == null)) {
            throw new UnknownHostException(
                    "The skeleton address is a wildcard and a port is assigned, but no address can be found for the local host");
        }
    ...
    public static <T> T create(Class<T> c, Skeleton<T> skeleton, String hostname) {
        ...
        // create stub
        T result = (T) (Proxy.newProxyInstance(c.getClassLoader(), new Class[] { c }, new ProxyClass<T>(new InetSocketAddress(
                hostname, skeleton.sockaddr.getPort()), c)));
        return result;
    }
