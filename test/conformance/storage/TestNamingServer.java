 ...
 * 
 * <p>
 * This naming server performs the following checks each time a storage server registers:
 * <ul>
 * <li>None of the arguments to <code>register</code> are <code>null</code>.</li>
...
public class TestNamingServer implements naming.Registration {
     ...
     * 
     * @param test
     *            The test which is running this naming server.
     */
     ...
     * 
     * @param files
     *            The files to expect. The naming server will check that these are indeed the files that are received. If this
     *            argument is <code>null</code>, the naming server will not perform the check.
     */
     ...
     * 
     * @param files
     *            The files to be deleted. If this argument is <code>null</code>, the naming server will not command the storage
     *            server to delete any files.
     */
    ...
    @Override
    public synchronized void register(Storage client_stub, Command command_stub, Path[] files) throws RMIException {
        // Ensure that none of the arguments are null.
        if (client_stub == null) {
            test.failure(new TestFailed("storage server client interface " + "null during registration"));
        }

        if (command_stub == null) {
            test.failure(new TestFailed("storage server command interface " + "null during registration"));
        }

        if (files == null) {
            test.failure(new TestFailed("files array null during " + "registration"));
        }
    ...
    private class TestSkeleton extends Skeleton<Registration> {
        ...
        /**
         * Sets the <code>stopped</code> flag and wakes any thread waiting in the server's <code>stop</code> method.
         */
        @Override
        ...
        protected boolean listen_error(Exception e) {
            test.failure(new TestFailed("caught exception in naming server " + "listening thread", e));
        ...
        protected void service_error(RMIException e) {
            test.failure(new TestFailed("caught exception in naming server " + "service thread", e));
        }
    }
}
